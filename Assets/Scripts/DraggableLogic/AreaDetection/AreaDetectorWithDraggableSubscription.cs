...
namespace Combat
{
    public abstract class AreaDetectorWithDraggableSubscription<T> : AreaDetector<T> where T: MonoBehaviour
    {
        ...
        public Action<T> PlacedComponentAdded;
    
        public Action<T> PlacedComponentRemoved;
        
        [SerializeField] private bool _componentHasHealth;
        
        private List<EntityHealth> _componentsHealth = new List<EntityHealth>();
        public Action<EntityHealth> HealthComponentAdded;
        public Action<EntityHealth> HealthComponentRemoved;
    
        private void Awake()
        {
            AddedItem += OnComponentAdded;
            RemovedItem += OnComponentRemoved;
        }
        ...
        private void RemovePlacedDraggable(DraggableObject draggable)
        {
            ...
            if (_componentHasHealth)
            {
                ...
                _componentsHealth.Remove(entityHealth);
                //entityHealth.Died.RemoveListener(RemoveDestroyedDraggable);
                HealthComponentRemoved.Invoke(entityHealth);
            }
        }
